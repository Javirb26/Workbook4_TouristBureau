...
window.onload = function categories() {
    ...
    // END OF: activities array

    //CATEGORY FOR LOOP

        // looping through catergories array to populate select element in HTML
    ...
    // FUNCTION onchange for activies select element
        categoryEl.onchange= function(){
            // value of category select el 
            const selectedCategory = categoryEl.value;
            // looping through activies array to populate select element for activities depending on conditional
            for(let i = 0; i < activities.length; i++){
                const activity = activities[i];
                // if the categories are the same then popullate activity select element with the name of the same category activies
